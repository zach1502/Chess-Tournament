...
// 4. quicience search NOT WORKING but can be enabled
// 5. futility pruning
// 6. move ordering*
// 7. opening book
// 8. Material table*
...
function trollFishAlgorithm (chess, turn, alpha, beta, maxDepth){
    positionCount++;
    ...
    if(turn){
        ...
        for(var i = 0; i < moves.length; i++){
            chess.move(moves[i]);
            bestMove = Math.max(bestMove,  trollFishAlgorithm(chess, !turn, alpha, beta, maxDepth - 1));
            chess.undo();
    ...
    // as black
        ...
        for(var i = 0; i < moves.length; i++){
            chess.move(moves[i]);
            bestMove = Math.min(bestMove,  trollFishAlgorithm(chess, !turn, alpha, beta, maxDepth - 1));
            chess.undo();
...
function trollFishAI(chess, turn, maxDepth){
    ...
    if(!turn){
        ...
        for(var i = 0; i < moves.length; i++){
            chess.move(moves[i]);
            moveScore =  trollFishAlgorithm(chess, turn, -99999, 99999, maxDepth-1, turn);
            chess.undo();
